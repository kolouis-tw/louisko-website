...
- `server/canonical-service.cjs`: 將既有網站 Bazi Core 與終身 Prompt Builder 以伺服器 adapter 供 API／LINE 共用。
- `server/line-bot.cjs`: LINE 指令解析、簽章驗證、訊息分段與互動訊息工具，不含八字計算。
- `scripts/line-bot-regression-check.cjs`: LINE 與 canonical generation 回歸測試，可用 `npm run test:bazi:line` 執行。
...
忘記密碼信的連結會以 `?reset=...` 直接開啟命主紀錄的重設密碼模式，不需要使用者先切換分頁；畫面提供新密碼、確認新密碼與「顯示新密碼」選項，成功後會清除 reset token 並回到一般帳號狀態。
...
LINE Bot 是現有 Node service 的私有 owner-scoped adapter，正式 webhook 為 `https://louisko.com/api/line/webhook`。它沿用網站 Bazi Core、既有終身 AI Prompt Builder、帳號 owner 與 R2／本機保存層，不另建 LINE 專用計算或資料庫。完整的環境變數、LINE Developers 設定、命主隔離、Prompt 讀取／重新產生差異、Markdown 短效下載 token、部署與 rollback 檢查請看：

```text
docs/agent-governance/line-bot-operations.md
```

必要環境變數只放 Zeabur secrets／environment：

```text
BAZI_LINE_OWNER_EMAIL
BAZI_LINE_CHANNEL_SECRET
BAZI_LINE_CHANNEL_ACCESS_TOKEN
BAZI_LINE_ALLOWED_USER_IDS
```

未設定 LINE 變數時 webhook 會 fail-closed，不影響網站排盤與原有 Prompt 匯出。
