...
完成排盤後，命主資料會優先同步至同一 Node 服務的 `/api/bazi/profiles`，由現有 R2 儲存層保存；`louisko_bazi_profiles_v1` 仍作為本機快取與 API 暫時不可用時的 fallback。紀錄包含原始輸入曆法、標準陽曆出生時間、農曆年月日與閏月、性別、南北半球、換日法、時區與建立／更新時間。同一姓名、出生時間、性別、出生地與換日法再次排盤時只更新 `updatedAt`，不建立重複資料。

API 以瀏覽器產生的 owner key 分開資料，避免沒有登入時不同訪客互相看到命主紀錄。這個版本是雲端保存但尚未提供帳號登入或跨裝置同步碼；清除瀏覽器資料後仍會失去原 owner key，後續若要跨裝置取回，需再加入登入或明確的同步碼流程。
